...
def train_one_epoch(train_dataloader, model, optimizer, loss):
    ...
    for batch_idx, (data, target) in tqdm(
        ...
        loss_value  = torch.nn.functional.nll_loss(output, target)
...
def optimize(data_loaders, model, optimizer, loss, n_epochs, save_path, interactive_tracking=False):
    ...
    scheduler  = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, "min", verbose=True, threshold=0.07)
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        # If the validation loss decreases by more than 5%, save the model
        if valid_loss_min is None or (
                (valid_loss_min - valid_loss) / valid_loss_min > 0.05
        ):
...
def one_epoch_test(test_dataloader, model, loss):
    ...
    with torch.no_grad():
        ...
        for batch_idx, (data, target) in tqdm(
            ...
            logits  = model(data)
            # 2. calculate the loss
            loss_value  = loss_value  = torch.nn.functional.nll_loss(logits, target)
            ...
            correct += torch.sum(torch.squeeze(pred[1]).eq(target.data.view_as(pred[1])).cpu())
            total += data.size(0)
